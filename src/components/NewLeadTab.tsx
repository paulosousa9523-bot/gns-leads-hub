...
export function NewLeadTab({ session }: { session: Session }) {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (error || !data) {
      setSaving(false);
      const isDupProcesso = (error?.message || "").includes("já cadastrado no CRM");
      setMsg(isDupProcesso ? `Número de processo duplicado — ${error?.message}` : "Erro: " + (error?.message || "falha ao salvar"));
      return;
    }
  ...
  return (
    <div className="bg-muted/30 border border-border rounded-lg p-2.5 space-y-2">
      ...
      <div className="flex gap-2">
        <label className="flex-1 inline-flex items-center justify-center gap-1 bg-primary text-primary-foreground text-xs font-semibold px-2.5 py-2 rounded-md cursor-pointer">
          <Upload className="w-3 h-3" /> Anexar documentos
